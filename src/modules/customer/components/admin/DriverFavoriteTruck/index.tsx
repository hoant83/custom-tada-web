...
const DriverFavoriteTruck = (props: ComponentProps) => {
   ...
   * Props of Component
   */
  const { style, className, children, customer } = props;
  ...
  const handleSubmitForm = async (values: any) => {
    const result = await customerStore.addFavoriteTruckOwnerByAdmin(
      values.publicId,
      +customer.id
    );
    if (result) {
      customerStore.getFavoriteTruckOwnerByAdmin(criteriaDto, +customer.id);
      toast.dismiss();
  ...
  const handleOk = async () => {
    ...
    if (deleteID) {
      const result = await customerStore.deleteFavoriteTruckOwnerByAdmin(
        deleteID,
        +customer.id
      );
      if (result) {
  ...
  const handleReset = async () => {
    const result = await customerStore.resetFavoriteTruckOwnerByAdmin(
      +customer.id
    );
    if (result) {
      customerStore.getFavoriteTruckOwnerByAdmin(criteriaDto, +customer.id);
...
export default observer(DriverFavoriteTruck);
