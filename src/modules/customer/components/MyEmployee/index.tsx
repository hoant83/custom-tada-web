...
const MyEmployee = (props: ComponentProps) => {
   ...
   * Props of Component
   */
  const { style, className, children, title, ownerAccountType } = props;
  ...
  const handleEdit = async (id: number) => {
    const employee = await customerStore.getEmployeeById(id);
    if (employee) {
  ...
  const handleSubmit = async (values: any) => {
    customerStore.setEmployeeForm({ ...values, accountType: ownerAccountType });

    if (mode === ACTION_MODE.CREATE) {
      const result = await customerStore.addEmployee();
      if (result) {
        setInfo(newEmployeeFormInit);
        customerStore.getEmployees(criteriaDto);
    ...
    if (mode === ACTION_MODE.EDIT) {
      const result = await customerStore.updateEmployee(updateID);
      if (result) {
        setInfo(newEmployeeFormInit);
        customerStore.getEmployees(criteriaDto);
        customerStore.resetEmployeeForm();
  ...
  const handleOk = async () => {
    ...
    if (deleteID) {
      const result = await customerStore.deleteEmployee(deleteID);
      if (result) {
        setDeleteID(-1);
        toast.dismiss();
                ...
                <tbody>
                  {customerStore.employees.map((item: any, index: number) => (
                    <tr key={item.id}>
                      ...
                      <td data-th={`${t(CUSTOMER_PHONE)}: `}>
                        {item.phoneNumber}
                      </td>
                      <td data-th={`${t(CUSTOMER_ROLE_LABEL)}: `}>
                      ...
                      <td className="col-actions">
                        {authStore.loggedUser?.accountRole !== 1 &&
                          item.accountRole !== 0 &&
                          item.id !== authStore.loggedUser?.id && (
                            <>
                              <Button
...
export default observer(MyEmployee);
