...
const CustomerStatisticsGrid = (props: ComponentProps) => {
  /*
   * Translation
   */
  ...
  const {
    ORDER_NO_ITEM,
    FILTER_EXPORT,
    REPORT_CUSTOMER_STATISTICS_TITLE,
    REPORT_EMAIL,
    REPORT_PHONE,
    REPORT_NAME,
    REPORT_COMPANY_NAME,
    REPORT_COMPLETED_ORDER,
    REPORT_TOTAL_INDIVIDUAL_REVENUE,
  ...
  const headingExport = [
    ...
    { header: t(REPORT_TOTAL_INDIVIDUAL_REVENUE), key: 'individualTotalPrice' },
  ...
  const getData = async (criteriaDto: any) => {
    const data = await adminService.getCustomerStatistics(criteriaDto);
    setItems(data[0]);
    setTotals(data[1]);
  ...
  const handleExport = async (type: TYPE_EXPORT) => {
    const data = await adminService.getCustomerStatistics({
      ...criteriaDto,
      isExportAll: true,
    });
    if (type === TYPE_EXPORT.XLS) {
      exportExcel(headingExport, data[0], 'customer-statistics');
    } else {
      exportCSV(headingExport, data[0], 'customer-statistics');
    }
  };
        ...
        <Row>
          <Col xs={12}>
            <h3 className="block-title">
              {t(REPORT_CUSTOMER_STATISTICS_TITLE)}
            </h3>
          </Col>
          <Col xs={12} md={6}>
            <FilterBy
            ...
            <Table className="table-wrapper">
              <thead>
                <tr>
                  ...
                  />
                  <SortTH
                    title={t(REPORT_TOTAL_INDIVIDUAL_REVENUE)}
                    name="individualTotalPrice"
                    onClick={() => setSort('individualTotalPrice')}
                    sort={{
                      orderBy: criteriaDto.orderBy,
                      orderDirection: criteriaDto.orderDirection,
                    }}
                ...
                {items && items.length > 0 ? (
                  items.map((item: any, index: number) => (
                    <tr>
                      ...
                      <td>
                        {item.individualTotalPrice
                          ? item.individualTotalPrice.toLocaleString()
                          : 0}
                      </td>
                      <td>
...
export default observer(CustomerStatisticsGrid);
