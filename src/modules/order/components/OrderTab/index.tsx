...
import { OrderStoreContext } from '../../order.store';
...
const OrderTab = (props: ComponentProps) => {
  ...
  const handleTracking = React.useCallback((order: any) => {
    ...
    if (orderById) {
      ...
      orderById.dropOffFields.map((dropOffField: any) => {
        const isString = typeof dropOffField === 'string';
        if (dropOffField.dropoffAddress) {
          tmpMarkers.push({
            lat: isString
              ? +JSON.parse(dropOffField).dropoffAddress[0]
              : +dropOffField.dropoffAddress[0],
            lng: isString
              ? +JSON.parse(dropOffField).dropoffAddress[1]
              : +dropOffField.dropoffAddress[1],
          });
        }
      });
