...
const MyDriver = (props: ComponentProps) => {
  ...
  const handleEdit = async (id: number) => {
    const driver = await truckOwnerStore.getDriverById(id);
    if (driver) {
  ...
  const uploadDriverFiles = async (id: number) => {
    if (cardFront.file) {
      await truckOwnerStore.uploadDriverCardFront(cardFront.file, id);
    }
    if (cardBack.file) {
      await truckOwnerStore.uploadDriverCardBack(cardFront.file, id);
    }
    if (licenseCard.file) {
      await truckOwnerStore.uploadDriverLicense(licenseCard.file, id);
    }
    if (otherDocuments.files && otherDocuments.files.length) {
      await truckOwnerStore.uploadDriverOtherDoc(otherDocuments.files, id);
  ...
  const handleSubmitDriver = async (values: any) => {
    ...
    if (mode === ACTION_MODE.CREATE) {
      ...
      if (result) {
        ...
        if (data) {
          truckOwnerStore.getDrivers(criteriaDto);
        }
        setDriverInfo(newFormInit);
    ...
    if (mode === ACTION_MODE.EDIT) {
      ...
      if (result) {
        const data = await truckOwnerStore.updateDriver(updateID);
        if (data) {
          setDriverInfo(newFormInit);
          truckOwnerStore.getDrivers(criteriaDto);
          truckOwnerStore.resetDriverForm();
  ...
  const handleOk = async () => {
    ...
    if (deleteID) {
      const result = await truckOwnerStore.deleteDriver(deleteID);
      if (result) {
        setDeleteID(-1);
        toast.dismiss();
  ...
  React.useEffect(() => {
    truckOwnerStore.getDrivers(criteriaDto);
  }, [authStore, truckOwnerStore, criteriaDto]);

  React.useEffect(() => {
    setTotalPage(Math.ceil(truckOwnerStore.totalDrivers / +pageSizeOptions[1]));
  }, [truckOwnerStore.totalDrivers]);

  return (
    <>
      {truckOwnerStore.drivers && (
        <Container
          fluid
          ...
          <Row>
            ...
            <Col xs={12} className="block-content">
              <Table className="table-wrapper">
                ...
                <tbody>
                  {truckOwnerStore.drivers.map((item: any, index: number) => (
                    <tr key={item.id}>
              ...
              {totalPage > 1 && (
                <>
                  <Paging
                    totalPage={totalPage}
                    totals={truckOwnerStore.totalDrivers}
