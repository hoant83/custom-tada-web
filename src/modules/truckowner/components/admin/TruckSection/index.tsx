...
const TruckSection = (props: ComponentProps) => {
  ...
  const handleEdit = async (id: number) => {
    const truck = await truckOwnerStore.getTruckByIdByAdmin(id);
    if (truck) {
  ...
  const uploadFiles = async (id: number) => {
    if (certificate.file) {
      await truckOwnerStore.uploadCertificateByAdmin(certificate.file, id);
    }
  ...
  const handleSubmit = async (values: any) => {
    ...
    if (mode === ACTION_MODE.CREATE) {
      const result = await truckOwnerStore.addTruckByAdmin(
        truckOwnerStore.truckOwnerAdmin.id
      );
      if (result) {
        const data = await uploadFiles(result.id);
        if (data) {
          truckOwnerStore.getTrucksByAdmin(
            truckOwnerStore.truckOwnerAdmin.id,
            criteriaDto
          );
        }
    ...
    if (mode === ACTION_MODE.EDIT) {
      ...
      if (result) {
        const data = await truckOwnerStore.updateTruckByAdmin(updateID);
        if (data) {
          setInfo(newTruckFormInit);
          truckOwnerStore.getTrucksByAdmin(
            truckOwnerStore.truckOwnerAdmin.id,
  ...
  const handleOk = async () => {
    ...
    if (deleteID) {
      const result = await truckOwnerStore.deleteTruckByAdmin(deleteID);
      if (result) {
        setDeleteID(-1);
        toast.dismiss();
  ...
  const handleDeleteImage = async (type: IMAGE_TYPE) => {
    if (type === IMAGE_TYPE.CERTIFICATE) {
      const result = truckOwnerStore.deleteTruckFileByAdmin(
        updateID,
  ...
  React.useEffect(() => {
    setTotalPage(
      Math.ceil(truckOwnerStore.totalAdminTrucks / +pageSizeOptions[1])
    );
  }, [truckOwnerStore.totalAdminTrucks]);

  return (
    <>
      {truckOwnerStore.myAdminTrucks && (
        <Container
          fluid
            ...
            <Col xs={12} className="block-content">
              ...
              {totalPage > 1 && (
                <>
                  <Paging
                    totalPage={totalPage}
                    totals={truckOwnerStore.totalAdminTrucks}
...
export default observer(TruckSection);
