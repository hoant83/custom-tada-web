...
const TruckownerGridAdmin = (props: ComponentProps) => {
  ...
  const handleSelectedAll = (checked: boolean) => {
    ...
    if (!checked) {
      ...
      truckOwnerStore.truckowners.forEach((item: any) => {
  ...
  React.useEffect(() => {
     ...
     */
    let tmpItems: any[] = [];
    setItems(truckOwnerStore.truckowners);
    truckOwnerStore.truckowners.map((item: any) => {
  ...
  }, [truckOwnerStore.truckowners, totals, isSelectedAll]);
              ...
              <Table responsive="lg">
                <thead>
                  <tr>
                    ...
                    <th>{t(CUSTOMER_PHONE_STATUS_LABEL)}</th>
                    <th>{t(COMPANY_NAME)}</th>
                    <th>{t(ADMIN_LAST_ACTIVE_DATE)}</th>
                    <th>{t(ADMIN_CREATED_DATE)}</th>
                    <th>{t(ACCOUNT_REFERAL_CODE)}</th>
                ...
                <tbody>
                  {items.map((item: any, index: number) => (
                    <tr key={item.id}>
                      ...
                      <td>{getPhoneVerifiedStatus(t, item.phoneVerified)}</td>
...
export default observer(TruckownerGridAdmin);
