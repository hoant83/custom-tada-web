...
const DeletedTruckOwnerGridAdmin = (props: ComponentProps) => {
  ...
  React.useEffect(() => {
     ...
     */
    let tmpItems: any[] = [];
    setItems(truckOwnerStore.deletedTruckOwners);
    items.map((item: any) => {
      tmpItems.push({ id: item.id, checked: false });
      return items;
    });
    setTotalPage(Math.ceil(totals / +pageSizeOptions[1]));
  }, [items, truckOwnerStore.deletedTruckOwners, totals]);
...
export default observer(DeletedTruckOwnerGridAdmin);
