...
import DeletedTruckOwnerGridAdmin from '@/modules/truckowner/components/DeletedGrid';
...
const ManageDeletedTruckOwnerAdminPage = () => {
  ...
  const {
    TRUCKOWNER_MANAGE_RESTORE_TITLE,
    CUSTOMER_EMAIL,
    CUSTOMER_PHONE,
    CUSTOMER_TYPE_LABEL,
  ...
  const handleOk = async () => {
    ...
    if (restoreID) {
      const result = await truckOwnerStore.restoreTruckOwnerByIdByAdmin(
        restoreID
      );
      if (result) {
        setRestoreID(-1);
        ...
        truckOwnerStore.getDeletedTruckOwnerByAdmin(criteriaDto);
      }
    }
  };
  ...
  return (
    <>
      <AdminWrapper pageTitle={t(TRUCKOWNER_MANAGE_RESTORE_TITLE)}>
        ...
        />
        <DeletedTruckOwnerGridAdmin
          totals={truckOwnerStore.totalDeletedTruckOwners}
