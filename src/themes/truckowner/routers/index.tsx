...
const TruckOwnerRouter = () => {
  ...
  React.useEffect(() => {
    const title: string = getRouteTitle(
      location.pathname,
      truckOwnerThemeRoutes
    );
    document.title = t(title);

    const route: string = getRoute(location.pathname, truckOwnerThemeRoutes);
  ...
  React.useEffect(() => {
    ...
    if (!existedToken && isGuarded(location.pathname, truckOwnerThemeRoutes)) {
    ...
    if (
      ...
      if (location.pathname === '/') {
        history.push(TRUCKOWNER_ROUTERS.SETUP);
        commonStore.setActiveMenu(TRUCKOWNER_ROUTERS.SETUP);
      }
    }
